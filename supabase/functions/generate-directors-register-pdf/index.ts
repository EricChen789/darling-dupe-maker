...
serve(async (req) => {
  ...
  try {
    ...
    const [companyRes, rolesRes, fontResp] = await Promise.all([
      supabase.from("companies").select("*").eq("id", companyId).single(),
      supabase.from("person_company_roles").select("*").eq("company_id", companyId).in("role", ["director", "secretary"]),
    ...
    const directors = roles.filter((r) => r.role === "director");
    const secretaries = roles.filter((r) => r.role === "secretary");
