  ...
  try {
    ...
    if (piPersons.length > 0) {
      ...
      for (let pi = 0; pi < piPersons.length; pi++) {
        ...
        //     Checkbox widgets retain their built-in appearances; we set /AS per widget.
        ...
        for (const pos of widgetPositions) {
          if (pos.fieldType === '/Tx') {
            ...
            // ── Checkbox: use widget's built-in appearance (no custom lines) ──
            ...
            if (pageAnnots && typeof pageAnnots.size === "function") {
              for (let j = 0; j < pageAnnots.size(); j++) {
                try {
                  ...
                  // Redraw checkbox border (original widget border may have been cleared)
                  page.drawRectangle({
                    x: pos.x, y: pos.y,
                    width: pos.w, height: pos.h,
                    borderWidth: 0.5,
